...
function displayClassic(messages) {
    ...
    if ( messages!=null && messages.length>0 ) {
    	if (strTrim(HEADER_MSG).length > 0) {
    		//MODIFY BY SKE 2008-3-26 BEGIN
            //str += HEADER_MSG + '\n\n';
            str += HEADER_MSG + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>';
            //MODIFY BY SKE 2008-3-26 END
        }
        ...
        //alert(str);
        //modify by yf 修改弹出alert为EXT的alert 2008-3-13 begin
        _alert(str);
        //modify by yf 修改弹出alert为EXT的alert 2008-3-13 end
        return false;
    } else {
    	return true;
    }
...
function checkRule(f, myRule) {
    ...
    if (myRule != null) {
        if (myRule.ruleName=='custom') {
        	...
        	//MODIFY BY SKE 2008-4-1 BEGIN
            //el = getField(f, myRule.el);
            els = getField(f, myRule.el);
            //if (el == null) {
            if (els == null) {
            ...
            if (els.length && !els.type){
            	for(ske1=0;ske1<els.length;ske1++){
            		el = els[ske1];
            		var t_err = null;
		            if(el.type) {
		                if(el.type=='hidden'||el.type=='text'||el.type=='password'||el.type=='textarea') {
		                    t_err = checkText(el, myRule);
		                } else if(el.type=='checkbox') {
		                    t_err = checkCheckbox(el, myRule);
		                } else if(el.type=='select-one') {
		                    t_err = checkSelOne(el, myRule);
		                } else if(el.type=='select-multiple') {
		                    t_err = checkSelMul(el, myRule);
		                } else if(el.type=='radio') {
		                    t_err = checkRadio(el, myRule);
		                } else {
		                    debug('DEBUG: type '+ el.type +' not supported');
		                }
		            } else {
...
function checkText(el, myRule) {
    ...
    /*ADD BY SKE 2008-5-19 BEGIN*/
    } else if (myRule.ruleName=='numeric_lt') {
    	err = checkNumerLessThen(el, myRule, false);
    } else if (myRule.ruleName=='numeric_le') {
    	err = checkNumerLessThen(el, myRule, true);
    /*ADD BY SKE 2008-5-19 END*/	
        ...
        err = checkDateLessThan2(el, myRule, false);
    } else if (myRule.ruleName=='date_le') {
        err = checkDateLessThan2(el, myRule, true);
    } else if (myRule.ruleName=='keypress') {
        // do nothing
    }
    //ADD BY SKE 2007-12-19 BEGIN
    else if (myRule.ruleName=='double_length') {
        err = checkDoubleLengthCheck(el, myRule);
    }
    //ADD BY SKE 2007-12-19 END
...
//ADD BY SKE 2007-12-19 BEGIN
function checkDoubleLengthCheck(el, myRule){
	maxLength = myRule.comparisonValue;
	lengths = maxLength.split(',');
	values = el.value.split('.');
	if (values.length == 1){
		if (values[0].length > lengths[0]){
			highlight(el, inputclasserror);
...
function checkDouble(el, myRule) {
    var sep = DECIMAL_SEP;
    //MODIFY BY SKE 2007-12-19 BEGIN
    reg = new RegExp("^[-+]{0,1}[0-9]+\\.[0-9]+|^[-+]{0,1}[0-9]+$");
    if (el.value == "")
    	return;
    //MODIFY BY SKE 2007-12-19 BEGIN
    if ( !reg.test(el.value) ) {
...
function checkDateLessThan2(el, myRule, isEqualAllowed) {
	error = null;
	if(el.value!=''){
	  var date =new Date(el.value.replace(/-/g, "/")); 
	  var endTime = myRule.comparisonValue.substr(0);
	  var dateMax =new Date(endTime.replace(/-/g, "/")); 
	  if(dateMax.getTime()-date.getTime()<0){
	  	   highlight(el, inputclasserror);
            error = myRule.alertMsg;
	  }
...
function checkNumerLessThen(el, myRule, isEqual) {
    error = null;
    var isMeta = myRule.comparisonValue.indexOf('$')==0 
        ? true
        : false;
...
function highlightMultipleClassName(el, clazz) {
    ...
    //) + " " + clazz );    
    if(el.length){
    	for (ske1=0;ske1<el.length;ske1++){
		    el[ske1].className = strTrim (
		    ( (typeof el[ske1].className != "undefined")
		        ? el[ske1].className.replace(re, "")
		        : ""
		    ) + " " + clazz );
	    }
    } else {
